...
- [ ] T001 [US1] In `handle()`, after the existing claim/release/`setNickname` sequence, capture
...
- [ ] T002 [US1] Integration test: a client with no channel memberships receives its own `NICK`
...
- [ ] T003 [US2] After the existing fan-out loop, when `echoToSender` is true and `target` isn't
  ...
  this same file)
- [ ] T004 [US2] Extend the existing `params().size() < 2` guard to also reject an empty `body`
  string with `412 ERR_NOTEXTTOSEND`, in the same file (depends on T003, same file)
- [ ] T005 [US2] Integration test: a client with echo-message negotiated sees its own DM echoed
...
- [ ] T006 [P] [US3] Change `PONG`'s reply to `List.of(serverName.get(), token)`; when
  ...
  (research.md "Story 3")
- [ ] T007 [P] [US3] Split the `LS, LIST -> handleLs(session)` case: `LIST` sends a new reply
...
- [ ] T008 [P] [US3] Change `NegotiationResult`'s `accepted`/`declined` fields from
...
- [ ] T009 [US3] Switch from `OutboundMessage.now(presentedForm, commandName, target, body)`
...
- [ ] T010 [P] [US3] Seed `render()`'s `tags` map with `message.clientTags()` before the
...
- [ ] T011 [P] [US3] Split the combined `lineBytes.length + 2 > MAX_LINE_LENGTH_BYTES` check
...
- [ ] T012 [US3] Change the invalid-UTF8 branch in `processLine()` from "reply `421`, return,
...
- [ ] T013 [US3] Integration tests: `PING`/`PONG` two-param shape and bare-`PING` `409`; `CAP
...
- [ ] T014 [P] [US4] Split the channel-name and (if present) key parameters on `,`, looping the
  ...
  (research.md "Story 4")
- [ ] T015 [US4] After the `JOIN` notification fan-out and before `sendNamesReply`, if
...
- [ ] T016 [P] [US4] Change the no-comment branch's default from `null` (omitted parameter) to
  `session.nickname()`, in
  `jircd-core/src/main/java/net/jircd/core/session/command/KickCommandHandler.java`
- [ ] T017 [P] [US4] Change the bare-ban-list-query condition to accept both `"b"` and `"+b"`,
  in `jircd-core/src/main/java/net/jircd/core/session/command/ModeCommandHandler.java`
- [ ] T018 [US4] Add a `NicknameRegistry` constructor parameter; in the `MEMBER`-kind branch of
...
- [ ] T019 [US4] Integration tests: `JOIN #one,#two` joins both; a joiner to a topic'd channel
...
- [ ] T020 [P] [US5] New handler: looks up each of up to 5 space-separated nicknames via
  ...
  (research.md "Story 5")
- [ ] T021 [P] [US5] New handler: sends a short, fixed `371 RPL_INFO` burst (server
...
- [ ] T022 [P] [US5] In the not-found branch, send `318 RPL_ENDOFWHOIS` (using the originally
...
- [ ] T023 [P] [US5] In the exact-nickname branch only, stop applying the shared `isVisibleTo`
...
- [ ] T024 [P] [US5] Change the clear-away condition to
...
- [ ] T025 [US5] Wire `UserhostCommandHandler`/`InfoCommandHandler` into the command dispatch
...
- [ ] T026 [US5] Integration tests: `USERHOST` returns host info; `INFO` returns `371`/`374` not
...
- [ ] T027 [US6] After the existing `381 RPL_YOUREOPER` send, enqueue an unsolicited
  ...
  (research.md "Story 6")
- [ ] T028 [US6] Integration test: a successful `OPER` results in both `381` and an unsolicited
  `MODE <nick> +o`, in
...
- [ ] T029 [P] Update `specs/001-ircv3-server/contracts/irc-protocol-commands.md`'s `NICK`,
...
- [ ] T031 [P] Code cleanup pass: confirm `./gradlew build` runs Spotless/SpotBugs/PMD clean
  across every touched module
- [ ] T032 Run the full `specs/005-fix-batch-conformance/quickstart.md` validation pass
...
- [ ] T033 Re-run the irctest suite (`github.com/jircd/irctest`'s `irctest.controllers.jircd`
