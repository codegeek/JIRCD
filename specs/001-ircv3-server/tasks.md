...
- [X] T085 [P] [US2] Integration test: `CAP LS 302` returns exactly `message-tags`, `server-time`, `echo-message` — no `sasl` or other capability (FR-025) in `jircd-integration-tests/src/test/java/net/jircd/integration/Story2CapabilityListTest.java`
...
- [X] T135 [US6] Implement the `EXTENSION` command handler (`ENABLE`/`DISABLE` via `ExtensionRegistry`; `481` if unprivileged; specific error for unknown or non-toggleable ids) in `jircd-server-extensions/admin/src/main/java/net/jircd/serverextensions/admin/ExtensionCommandHandler.java`
...
- [X] T137 [US6] Implement the `WHOHOST` command handler (reads `ClientSession.realHostname` directly, bypassing any cloak extension) in `jircd-server-extensions/admin/src/main/java/net/jircd/serverextensions/admin/WhohostCommandHandler.java`
...
- [X] T140 [US6] Implement the `admin` `ServerExtension` wiring, registering the `OPER`/`EXTENSION`/`REHASH`/`WHOHOST`/`SAJOIN`/`SAMODE` handlers with `jircd-core`'s command dispatch when enabled (depends on T134-T137, T138, T139) in `jircd-server-extensions/admin/src/main/java/net/jircd/serverextensions/admin/AdminExtension.java`
- [X] T141 [US6] Implement the `cloak` `ServerExtension`, claiming the `hostname-display` extension point (research.md "Cloak extension boundary", FR-031) in `jircd-server-extensions/cloak/src/main/java/net/jircd/serverextensions/cloak/CloakExtension.java`
- [X] T142 [P] [US6] Register `admin` and `cloak` as `ServiceLoader` providers via `META-INF/services/net.jircd.core.extension.ServerExtension` in `jircd-server-extensions/admin/src/main/resources/` and `jircd-server-extensions/cloak/src/main/resources/`
...
- [X] T153 [P] Load-tagged (`@Tag("load")`) test: the server sustains 1,000 simultaneous connections without channel message delivery delay exceeding SC-002's target (SC-003) in `jircd-integration-tests/src/test/java/net/jircd/integration/ConcurrentConnectionScaleLoadTest.java`
- [X] T154 [P] Load-tagged (`@Tag("load")`) test: during a sustained flood from one connection, delivery latency for other well-behaved clients does not increase beyond SC-002's target (SC-006) in `jircd-integration-tests/src/test/java/net/jircd/integration/RateLimitLoadTest.java`
- [X] T155 [P] Update `README.md`'s "Getting started" section with real build/run instructions now that the project builds (`./gradlew build`, `./gradlew :jircd-server:run`), replacing the pre-implementation placeholder
- [X] T156 Run the full `specs/001-ircv3-server/quickstart.md` validation pass manually against a running `./gradlew :jircd-server:run` instance, covering Stories 1, 2, 4, 5, 6, and 7 end-to-end (constitution UX Consistency principle's required manual usage-scenario check)
- [X] T157 [P] Code cleanup pass: remove any dead code/TODOs introduced during implementation and confirm `./gradlew build` runs Spotless and SpotBugs clean across all subprojects (constitution Code Quality principle)
...
- [X] T158 Add a regression test proving a configured `rateLimit` (`bucketSize`/`refillRatePerSecond`) value is actually honored by new connections, both at startup and after `REHASH` — the exact bug fixed in Phase 9 (`ConnectionHandler` previously ignored `ServerConfiguration.rateLimit()` entirely) had no test proving the fix, in `jircd-integration-tests/src/test/java/net/jircd/integration/RateLimitConfigTest.java` per Constitution II (partial)
- [X] T159 Implement the still-unchecked T107: an integration test verifying a ban mask matching a channel member's real, uncloaked hostname mutes them despite the `cloak` extension presenting a different value in their message prefixes (FR-062's dual real-vs-presented matching, US5 Acceptance Scenario 8) in `jircd-integration-tests/src/test/java/net/jircd/integration/Story5BanRealHostnameTest.java` per FR-062 (missing)
- [X] T160 Extend `Story5PrivacyTest.java` (or add a new test) with administrator-visibility coverage: a privileged session viewing `TOPIC`/`NAMES`/`LIST` for a private/secret channel it is not a member of sees the channel normally, the same transparency guarantee already implemented in `ChannelVisibility.isHiddenFrom` — the existing test's own method name claims this coverage but its body never exercises an administrator session per FR-047 (partial)
- [X] T161 Add UTF-8 validation test coverage for FR-054: a unit test for `Utf8Validator` (`jircd-protocol`) covering well-formed and malformed byte sequences, plus integration coverage confirming invalid-UTF-8 rejection on `PRIVMSG`/`NOTICE` bodies, `TOPIC` text, `USER` realname, and `QUIT`/`PART` reasons in `jircd-protocol/src/test/java/net/jircd/protocol/Utf8ValidatorTest.java` per FR-054 (missing)

---

## Phase 11: Convergence

- [X] T162 Wire `LivenessMonitor` (FR-039) into real connection handling — it is fully implemented and unit-tested but never constructed or attached to a session anywhere in production (`ConnectionHandler`/`JircdServerApplication`): construct one per session with reasonable industry-standard probe-interval/timeout defaults (spec.md Assumptions — not administrator-configurable), call `session.attachLivenessMonitor(...)`, and drive periodic `checkNow()` ticks (e.g. one loop per connection, matching the existing per-connection virtual-thread architecture already used for the read loop) — without this, a connection that goes silent with no TCP-level signal is never detected or cleaned up, contradicting the contract's "Implemented" claim for server-initiated `PING` (`contracts/irc-protocol-commands.md` "Connection Keep-Alive") in `jircd-core/src/main/java/net/jircd/core/session/ConnectionHandler.java` per FR-039 (missing)
- [X] T163 Add an integration test for FR-049's protocol line-length limit: a raw line exceeding 512+4096 bytes (command+params, CR-LF inclusive, plus the message-tags allowance) is rejected with `417 ERR_INPUTTOOLONG`, distinct from `Story4LengthLimitConfigTest`'s topic-length case (FR-056) which reuses the same numeric for an unrelated requirement, in `jircd-integration-tests/src/test/java/net/jircd/integration/LineLengthLimitTest.java` per FR-049 (missing)
- [X] T164 Add an integration test for FR-039's client-initiated keep-alive: a client sends `PING <token>` at any time (including before registration completes) and receives an immediate `PONG <token>` reply, in `jircd-integration-tests/src/test/java/net/jircd/integration/PingPongTest.java` per FR-039 (missing)
