...
| `listeners` | list of {port, tlsEnabled, certPath?, keyPath?, keystorePath?, keystorePassword?} | Plaintext and/or TLS listeners (FR-018 — both may coexist). A `tls: true` listener MUST have exactly one certificate form configured — `certPath`+`keyPath` (PEM certificate/chain and private key) or `keystorePath`(+`keystorePassword`, PKCS12) — validated and loaded at startup, never both, never neither (004-fix-tls-certificate FR-001 through FR-006); a `tls: false` listener ignores any of the four fields if present. No certificate is ever generated automatically. |
